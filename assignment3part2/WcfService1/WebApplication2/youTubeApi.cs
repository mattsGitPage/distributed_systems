...
namespace WebApplication2
{
    public class youTubeApi
    {
        ...
        internal static YouTubeVideo[] getList(string simVidId)
        {
            var request = ytService.PlaylistItems.List("contentDetails");
            request.PlaylistId = simVidId;

            var response = request.Execute();

            LinkedList<YouTubeVideo> all = new LinkedList<YouTubeVideo>();
            ...
            while(nextPage != null)
            {
                ...
                {
                    all.AddLast(new YouTubeVideo(item.ContentDetails.VideoId));
                }
                nextPage = response.NextPageToken; 
            }
           
            return all.ToArray<YouTubeVideo>();
        }
    }
}